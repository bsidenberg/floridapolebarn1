...
export const LS_KEYS={components:"fpb_components",pricing:"fpb_pricing",inventory:"fpb_inventory",rules:"fpb_bom_rules"};
export function lsGet(k){try{return JSON.parse(localStorage.getItem(k))||[]}catch{return[]}}
...
// ══════════ SEED DATA (Backwoods Product Catalog) ══════════
const SEED_PRICING=[
  // TRUSSES
...
const SEED_COMPONENTS=SEED_PRICING.map(p=>({name:p.component_name,category:p.category,unit:p.unit,description:"",active:true}));
...
export const MGMT_CATS=["Posts","Trusses","Lumber","Panels","Trim","Fasteners","Concrete","Doors & Windows","Misc"];
export const CAT_ICONS={Posts:"🪵",Trusses:"🏗️",Lumber:"📐",Panels:"🔲",Trim:"✂️",Fasteners:"🔩",Concrete:"🧱","Doors & Windows":"🚪",Misc:"📦"};
export const UNITS=["Each","SF","LF","Bag","Sheet","CY","Box","Roll","Pair"];

export function autoCategory(name){
  const n=(name||"").toUpperCase();
  if(n.includes("TRUSS"))return"Trusses";if(n.includes("POST"))return"Posts";
  if(n.includes("PANEL")||n.includes("PBR")||n.includes("ULTRARIB")||n.includes("STANDING SEAM"))return"Panels";
  if(n.includes("TRIM")||n.includes("FASCIA")||n.includes("RAKE")||n.includes("DRIP")||n.includes("RIDGE")||n.includes("J-TRIM")||n.includes("CORNER"))return"Trim";
  if(n.includes("2X")||n.includes("1X")||n.includes("GIRT")||n.includes("PURLIN")||n.includes("SKIRT")||n.includes("SHEATH")||n.includes("OSB"))return"Lumber";
  if(n.includes("SCREW")||n.includes("BOLT")||n.includes("WASHER")||n.includes("NUT")||n.includes("REBAR")||n.includes("NAIL"))return"Fasteners";
  if(n.includes("CONCRETE")||n.includes("80LB")||n.includes("SLAB")||n.includes("BAG"))return"Concrete";
  if(n.includes("DOOR")||n.includes("WINDOW"))return"Doors & Windows";
  return"Misc"
}
...
export function Modal({open,onClose,title,children,footer}){
  ...
  return <div className="modal-overlay" onClick={e=>{if(e.target===e.currentTarget)onClose()}}>
    <div className="modal">
      <div className="modal-hdr"><h3>{title}</h3><button className="mgmt-btn sm" onClick={onClose}>✕</button></div>
...
export function parseCSV(text){
  ...
  return lines.slice(1).map(line=>{
    ...
    const obj={id:genId()};
    headers.forEach((h,i)=>{let v=(vals[i]||"").replace(/^"|"$/g,"").trim();
      if(v&&!isNaN(Number(v)))v=Number(v);obj[h]=v});
    return obj
  })
}
